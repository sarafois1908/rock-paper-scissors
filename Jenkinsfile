pipeline {
	...
	stages {
		...
		stage ('Push'){
			steps{
				dir("/opt/jenkins/rock-paper-scissors/target"){
					withCredentials([usernamePassword(credentialsId: 'cf6a9544-f46a-44d6-a9e1-33f8c9c02b6c', passwordVariable: 'nexus_password')]) {
						sh "curl -v -u admin:$nexus_password --upload-file $artifact_name http://172.29.16.32:8081/repository/M-PRV/$artifact_name"
					}
				}	
			}
		}
